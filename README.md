# Compiler Design (CD) Lab

A comprehensive collection of Lex and Yacc programs for the Compiler Design laboratory. This repository covers lexical analysis, syntax analysis, intermediate code generation, and target code generation.

---

## 📖 Core Concepts: Lex & Yacc

### 1. Lex: The Scanner (Tokenizer)
Lex (or Flex) is used to perform lexical analysis. it reads an input stream and groups characters into meaningful sequences called **tokens** based on regular expressions.

#### Program Structure (`.l` file)
Lex files are divided into three sections separated by `%%`:

```c
%{
  /* 1. Declarations */
  /* C code, #includes, and variable declarations */
%}

/* (Optional Lex definitions) */

%%
  /* 2. Translation Rules */
  /* Pattern (Regex)    { Action (C code) } */
%%

/* 3. Auxiliary C Functions */
/* Main function or helper logic */
```

### 2. Yacc: The Parser
Yacc (Yet Another Compiler-Compiler) or Bison performs syntax analysis. It takes tokens from Lex and arranges them according to a **Context-Free Grammar (CFG)**.

#### Program Structure (`.y` file)
Yacc files follow a similar three-part structure but include specific declarations for the parser.

```c
%{
  /* 1. C Declarations */
  #include <stdio.h>
%}

/* 1.5 Yacc Declarations */
%token NUM IDEN
%left '+' '-'

%%
  /* 2. Grammar Rules (CFG) */
  /* NonTerminal : Production { C Action } ; */
%%

/* 3. Auxiliary C Functions */
/* Usually main() and yyerror() */
```

### 3. Integration
When compiled together, Yacc calls the `yylex()` function (generated by Lex) to fetch the next token. Lex identifies the piece, and Yacc validates the overall structure of the code.

---

## 🚀 Lab Programs Index

Compact index for the Flex/Yacc exercises in this folder. The source code lives in the program directories; this file only explains what each one does and how to run it.

---

## Program 1

### 1a - Text Counter
Files: [Prog1/Lex/lex.l](Prog1/Lex/lex.l)

Counts lines, words, characters, and spaces until `#` is entered.

```bash
cd Prog1/Lex
flex lex.l
cc lex.yy.c -o prog1a -ll
./prog1a
```

### 1b - String Parser for a^n b^(n+m) c^m
Files: [Prog1/Yacc/lex.l](Prog1/Yacc/lex.l), [Prog1/Yacc/yacc.y](Prog1/Yacc/yacc.y)

Checks whether the input string belongs to the language a^n b^(n+m) c^m with n > 0 and m > 0.

```bash
cd Prog1/Yacc
yacc -d yacc.y
flex lex.l
cc lex.yy.c y.tab.c -o prog1b -ll
./prog1b
```

---

## Program 2

### 2a - Fraction & Integer Counter
Files: [Prog2/Lex/lex.l](Prog2/Lex/lex.l)

Counts positive integers, negative integers, positive fractions, and negative fractions.

```bash
cd Prog2/Lex
flex lex.l
cc lex.yy.c -o prog2a -ll
./prog2a
```

### 2b - Arithmetic Calculator
Files: [Prog2/Yacc/lex.l](Prog2/Yacc/lex.l), [Prog2/Yacc/yacc.y](Prog2/Yacc/yacc.y)

Parses arithmetic expressions with `+`, `-`, `*`, `/`, parentheses, and negative numbers, then prints the integer result.

```bash
cd Prog2/Yacc
yacc -d yacc.y
flex lex.l
cc lex.yy.c y.tab.c -o prog2b -ll
./prog2b
```

---

## Program 3

### 3a - For-Loop Depth Counter
Files: [Prog3/a/lex.l](Prog3/a/lex.l), [Prog3/a/yacc.y](Prog3/a/yacc.y)

Parses simplified C-like code, counts total `for` loops, and computes the maximum nesting depth using mid-rule actions.

```bash
cd Prog3/a
yacc -d yacc.y
flex lex.l
cc lex.yy.c y.tab.c -o prog3a -ll
./prog3a
```

### 3b - C Function Syntax Checker
Files: [Prog3/b/lex.l](Prog3/b/lex.l), [Prog3/b/yacc.y](Prog3/b/yacc.y)

Checks whether a C-like function declaration and body follow the supported syntax.

```bash
cd Prog3/b
yacc -d yacc.y
flex lex.l
cc lex.yy.c y.tab.c -o prog3b -ll
./prog3b
```

---

## Program 4 - Three Address Code and Quadruples
Files: [Prog4/lex.l](Prog4/lex.l), [Prog4/yacc.y](Prog4/yacc.y)

Parses arithmetic expressions and generates both three-address code and quadruples.

```bash
cd Prog4
yacc -d yacc.y
flex lex.l
cc lex.yy.c y.tab.c -o prog4 -ll
./prog4
```

---

## Program 5 - Target Code Generation
Files: [Prog5/lex.l](Prog5/lex.l), [Prog5/yacc.y](Prog5/yacc.y)

Parses arithmetic expressions and converts the intermediate form into simple target code instructions.

```bash
cd Prog5
yacc -d yacc.y
flex lex.l
cc lex.yy.c y.tab.c -o prog5 -ll
./prog5
```

---

---

## 🛠️ Build & Execution Guide

### Standard Build Pattern
For most programs in this repository, follow this sequence:

```bash
yacc -d yacc.y        # Generate parser source and header files
flex lex.l             # Generate scanner source file
cc lex.yy.c y.tab.c -o program -ll  # Compile and link
./program              # Run executable
```

> [!TIP]
> If you are using **Bison** instead of Yacc, the command is `bison -d -y yacc.y`.

---

## 📝 Exam Cheat Sheet (Skeletons)

### Lex Template (`lex.l`)

```c
%{
#include <stdio.h>
#include "y.tab.h"
%}

%option noyywrap

%%
/* ---> YOUR RULES GO HERE <--- */
%%
```

### Yacc skeleton (`yacc.y`)

```c
%{
#include <stdio.h>
#include <stdlib.h>

int yylex();
int yyerror();
%}

/* ---> %token DEFINITIONS GO HERE <--- */

%%
/* ---> YOUR GRAMMAR GOES HERE <--- */
%%

int main() {
    yyparse();
    return 0;
}

int yyerror() {
    printf("Error!\n");
    exit(0);
}
```

```PS: Download YASH extension for better readability```